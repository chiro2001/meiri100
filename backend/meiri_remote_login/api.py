...
class RemoteLoginAPI(Resource):
    args_check_account = reqparse.RequestParser() \
    ...
    @args_required_method(args_check_account)
    @auth_required_method
    def post(self, uid: int):
        username, password = self.args_check_account.parse_args().get('username'), \
                             self.args_check_account.parse_args().get('password')
        # 尝试登录
